...
def get_norm(norm, out_channels, **kwargs):
    ...
    if isinstance(norm, str):
        ...
        norm = {
            ...
            "StdN": StdNorm2d,
            "SyncBN": NaiveSyncBatchNorm,  # nn.SyncBatchNorm,
...
class StdNorm2d(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        return x / (var + self.eps).sqrt()


class StdNorm2dV2(nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, x: torch.Tensor):
        return x * torch.rsqrt(torch.mean(x ** 2, dim=(2, 3), keepdim=True) + 1e-8)
